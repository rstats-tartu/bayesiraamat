...
**Tõenäosusteooria aksioomid**, mida on kolm tükki, ütlevad tõlkes inimkeelde, (1) et iga sündmuse tõenäosus on suurem või võrdne nulliga, (2) et loogiliselt paratamatu sündmuse tõenäosus on üks (niiviisi sidudes tõenäosusteooria lausearvutusliku loogikaga) ja (3) et üksteist välistavate sündmuse puhul võrdub tõenäosus, et toimub üks või teine nende sündmuste tõenäosuste summaga ($P(A \lor B) = P(A) + P(B)$). Need aksioomid peaksid olema iseenesestmõistetavad ja ainult neist on tuletatud kogu tõenäosusteooria.  

Need aksioomid, mis oma matemaatilises vormis postuleeriti Andrei Kolmogorovi poolt ca 1933, on tuletatavad järgmistest eeldustest:

+ usutavuse määra saab kirjeldada reaalarvudega 0 ja 1 vahel
...
Kui tõenäosused on 0 või 1, siis taandub tõenäosusteooria matemaatliselt oma erijuhule, milleks on lausearvutuslik loogika. Lausearvutusel on huvitav omadus, et kui selle abil on saavutatud valiidne tulemus, on see tõsikindel ja uute andmete lisandumisel ei saa me seda tulemust muuta. Seevastu tõenäosusteoorias ja statstikas muudavad uued andmed alati tõenäosusi. Selles mõttes ei saa tõenäosuslik teadus kunagi valmis ja kui inimene on 100% veendunud mingite sündmuste tõesuses või vääruses, siis seisavad tema uskumused väljaspool teadust selles mõttes, et neid ei ole võimalik teaduslike argumentidega mõjutada.
...
1. konjunktsioon: $P(A \And B) \le P(A)$, ehk kui B on A alamhulk, siis $P(B) \leq P(A)$.  

2. Definitsioon: A ja B on üksteisest sõltumatud siis ja ainult siis kui $P(A~ \vert B) = P(A)$ 

3. Kui A ja B on üksteisest sõltumatud, siis $P(A \land B) = P(A)P(B) = P(A~\vert~B)P(B)$ 

4. Kui A ja B ei ole üksteist välistavad, siis $P(A \lor B) = P(A) + P(B) - P(A \And B)$. Kolmele sündmusele: $P(A \lor B \lor C) = P(A) + P(B) + (C) - P(A \And B) - P(B \And C) - P(A \And C) + P(A \And B \And C)$. 

5. Tingliku tõenäosuse definitsioon: $P(A~\vert~B) = \frac{P(A \land B)}{P(B)}$. 

6. Totaalne ehk marginaalne tõenäosus: $P(A) = P(A~\vert~B)P(B) + P(A~\vert~\lnot B)P(\lnot B)$, ehk $P(A) = P(A~\vert~B_1)P(B_1) + P(A~\vert~ B_2)P(B_2) + ...$ kõikidele võimalikele B-le, tuletatud 5. punktist.  

7. Bayesi teoreem: $P(A~\vert~B) =\frac{P(A)P(B~\vert~A)}{P(B)}$ -- tuletatud 5. punktist, kus $P(B) = P(A)P(B~\vert~A) + P(\lnot A)P(B~\vert~\lnot A)$ tuleb 6. punktist.     
...
Mida kitsam on posteerior, seda kitsamad tulevad sellest arvutatud usalduspiirid. Siit tuleneb, et kui võimalik peaksime oma mudelitesse panema parameetreid (statistikuid), mille posteeriorid tulevad maksimaalselt kitsad (vt allpool ptk "ajalooline vahepala" selle kohta, kuidas aritmeetiline keskmine on selline statistik).  
...
Teisisõnu, tõenäosusteooria õpetab meid tõenäosustega matemaatiliselt ümber käima, kuid ei anna meile seost matemaatiliste tõenäosuste ja päris maailma vahel, ega ei ütle, mida tõenäosus teaduses tähendab. Kaasajal eksisteerib kaks põhilist tõenäosuse tõlgendust, bayesiaanlik ja sageduslik, millest me siin käsitleme esimest. Sagedisliku tõlgenduse kohta vt lisa 1.
...
hõlpsalt kontrollida, kas ja kuidas meie mudelid töötavad, aga ka genereerida olukordi 
