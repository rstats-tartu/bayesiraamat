...
1. konjunktsioon: kui B on A alamhulk, siis $P(B) \leq P(A)$, ehk $P(A \And B) \le P(A)$ 
...
4. summareegel: kui A ja B on üksteist välistavad, siis $P(A \lor B) = P(A) + P(B)$. 

4.1. Kui A ja B ei ole üksteist välistavad, siis $P(A \lor B) = P(A) + P(B) - P(A \And B)$. Kolmele sündmusele: $P(A \lor B \lor C) = P(A) + P(B) + (C) - P(A \And B) - P(B \And C) - P(A \And C) + P(A \And B \And C)$ 

5. Kui A ja B ei ole üksteist välistavad, siis $P(A \lor B) = P(A) + P(B) - P(A \land B)$

6. Def: $P(A~\vert~B) = \frac{P(A \land B)}{P(B)}$ -- Tinglik tõenäosus

7. Totaalne ehk marginaalne tõenäosus: $P(A) = P(A~\vert~B)P(B) + P(A~\vert~\lnot B)P(\lnot B)$, ehk $P(A) = P(A~\vert~B_1)P(B_1) + P(A~\vert~ B_2)P( B_2) + ... + P(A~\vert~ B_n)P(B_n)$ kõikidele võimalikele B-le   -- , tuletatud 6. punktist.  

8. Bayesi teoreem: $P(A~\vert~B) =\frac{P(A)P(B~\vert~A)}{P(B)}$ -- tuletatud 6. punktist --, kus $P(B) = P(A)P(B~\vert~A) + P(\lnot A)P(B~\vert~\lnot A)$  -- 7. punktist.     
