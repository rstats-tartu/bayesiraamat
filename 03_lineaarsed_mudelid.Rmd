...
Lihtsaim mudel pikkuse sõltuvusest kaalust on pikkus = kaal (formaliseeritult: y = x) ja see mudel ennustab, et kui Johni kaal = 80 kg, siis John on 80 cm pikkune. 
...
```{r, fig.cap="Lihtne mudel y ~ x, mille intecept = 0 ja tõus = 1.", fig.height= 3, fig.width=3}
plot(y ~ x, type = "l", xlab = "Weight in kg", ylab = "Heigth in cm")
```
...
```{r, fig.cap="y ~ a + x mudel, mille intecept = a ja tõus = 1.", fig.height= 3, fig.width=3}
plot(y ~ x, xlim = c(0, 100), ylim = c(0, 150), col = "red", type = "l")
abline(c(0, 1), lty = 2)
```
...
```{r fig.cap="y ~ bx mudel, mille intercept = 0 ja tõus = b", fig.height= 3, fig.width=3}
plot(y ~ x, xlim = c(0, 100), ylim = c(0, 100), col = "red", type = "l")
abline(c(0, 1), lty = 2)
```
...
```{r fig.cap="y ~ a + bx mudel, milles saab tuunida nii intercepti kui tõusu.", fig.height= 3, fig.width=3}
plot(y ~ x, xlab = "Weight in kg", ylab = "Heigth in cm", ylim = c(50, 200), type = "l")
```
...
```{r fig.cap="Fititud mudel, kus muutuja Petal.Length järgi ennustatakse muutuja Sepal.Length väärtusi.", fig.height=4, fig.width=6}
...
```{r, fig.cap="Siin ennustasime 10 Sepal_length väärtust 10-l Petal_length väärtusel.", fig.height=4, fig.width=6}
...
```{r, echo=FALSE, fig.cap="Kasvava paindlikusega polünoomsed mudelid. mod_e1 on sirge võrrand y = a + b~1~x (2 parameetrit: a ja b~1~), mod_e2 on lihtsaim võimalik polünoom: y= a + b~1~x + b~2~x^2 (3 parameetrit), ..., mod_e5: y= a + b~1~x + b~2~x^2 + b~3~x^3 + b4x^4 + b5x^5 (6 parameetrit). mod_e5 vastab täpselt andmepunktidele (N = 6).", fig.height=4, fig.width=6}
library(viridis)
tib %>% 
...
```{r echo=FALSE}
tib %>% select(model_formula, aic) %>% knitr::kable()
```
