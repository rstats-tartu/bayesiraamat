...
```{r, fig.cap="Siin ennustasime 10 y väärtust (Sepal_length) 10-l x-i väärtusel (Petal_length)."}
...
```{r, echo=FALSE}
tib <- tibble(x = c(2, 3, 2.25, 5, 7.3, 8), y = c(8, 5.1, 9.5, 10.5, 11, 10.1))
mod_e1 <- lm(y ~ x, data = tib)
mod_e2 <- lm(y ~ poly(x, 2), data = tib)
mod_e3 <- lm(y ~ poly(x, 3), data = tib)
mod_e4 <- lm(y ~ poly(x, 4), data = tib)
mod_e5 <- lm(y ~ poly(x, 5), data = tib)
## Extract model formulas for plotting
model_formulas <- tibble(model = c("mod_e1", "mod_e2", "mod_e3", "mod_e4", "mod_e5"),
                         fit = list(mod_e1, mod_e2, mod_e3, mod_e4, mod_e5)) %>% 
  mutate(frms = map(fit, formula),
         frms = map_chr(frms, deparse)) %>% 
  select(model, frms)
```


```{r, echo=FALSE, fig.cap="Kasvava paindlikusega polünoomsed mudelid. mod_e1 on sirge võrrand y = a + b~1~x (2 parameetrit: a ja b~1~), mod_e2 on lihtsaim võimalik polünoom: y= a + b~1~x + b~2~x^2 (3 parameetrit), ..., mod_e5: y= a + b~1~x + b~2~x^2 + b~3~x^3 + b4x^4 + b5x^5 (6 parameetrit). mod_e5 vastab täpselt andmepunktidele (N = 6)."}
tib %>%
  gather_predictions(mod_e1, mod_e2, mod_e3, mod_e4, mod_e5) %>% 
  left_join(model_formulas) %>%
  ggplot() +
  geom_point(aes(x, y), color = "black") +
  geom_line(aes(x, pred, colour = frms)) +
