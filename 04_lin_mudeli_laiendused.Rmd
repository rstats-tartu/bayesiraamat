...
Kui meie andmed on mõõdetud 3D-s ja me tahame ennnustada ühe muutuja väärtust kahe teise muutuja väärtuste põhjal (meil on 2 prediktorit), siis tuleb meie 3 parameetriga lineaarne regressioonimudel tasapinna kujul. 
Kui meil on 4 prediktoriga mudel, siis me liigume 4-mõõtmelisse ruumi, jne. 
3D ruumi on veel võimalik mõistlikult plottida.

```{r echo=FALSE}
m2 <- lm(Sepal.Width ~ Sepal.Length + Petal.Length, data = iris)
```

```{r, echo=FALSE, fig.cap='Kahe prediktoriga lineaarse mudeli 3D representatsioon.'}
...
```{r lin2d, out.width='0.49\\linewidth', fig.cap='Kahe prediktoriga lineaarne mudel kahes dimensioonis.'}
...
```{r, fig.cap='Interaktsioonimudel 2Ds.'}
Petal_length <-  mean(iris$Petal.Length)
a <- coef(m3)[1]
b1 <- coef(m3)[2]
b2 <- coef(m3)[3]
...
kus $\mu$ on mudeli poolt ennustatud keskväärtus ja $\sigma$ on mudeli poolt ennustatud standardhälve ehk varieeruvus andmepunktide tasemel. 
...
```{r fig.height=2, fig.width=4, fig.cap='Kümne tuhande N = 12 suuruse  juhuvalimi summa tihedusdiagramm.'}
library(rethinking)
...
```{r fig.height=2, fig.width=4, fig.cap='Kümne tuhande N = 12 suuruse juhuvalimi korrutiste tihedusdiagramm. Ühegi geeni mõju ei domineeri teiste üle.'}
...
```{r fig.height=2, fig.width=4, fig.cap='Kümne tuhande N = 12 suuruse juhuvalimi korrutiste tihedusdiagramm. Mõnel geenil on kuni 2 korda suurem mõju kui teisel.'}
...
```{r fig.height=2, fig.width=4, fig.cap='Logaritmilises skaalas. Kümne tuhande N = 12 suuruse juhuvalimi korrutiste tihedusdiagramm. Mõnel geenil on kuni 2 korda suurem mõju kui teisel.'}
...
```{r message=FALSE, fig.height=3, fig.width=4, fig.cap='Juhuvalim normaaljaotusest, mille keskmine=0 ja sd=1 (n=3; andmepunktid on näidatud mustade munadena). Sinine joon - popualtsioon, millest tõmmati valim; punane joon - normaaljaotuse mudel, mis on fititud valimi andmetel; must joon - Studenti t jaotuse mudel, mis on fititud samade andmetega.'}
library(brms)
set.seed(19)
dfr <- tibble(a = rnorm(3))

dfr %>% 
  ggplot(aes(a)) +
  geom_dotplot() + 
  stat_function(fun = dnorm, 
                args = list(mean = mean(dfr$a),
                            sd = sd(dfr$a)), colour = "red") +
...
```{r fig.height=3, fig.width=4, fig.cap='Mis juhtub kui me kasutame oma normaaljaotuse mudelit uute andmete simuleerimiseks? Kui lähedased on need simuleeritud andmed populatsiooni andmetega ja kui lähedased valimi andmetega, millega me normaaljaotuse mudeli fittisime.'}
