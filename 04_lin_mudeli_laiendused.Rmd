...
3D ruumi on võimalik visualiseerida 2D ruumis, kui kollapseerida kolmas mõõde konstandile. 

```{r}
m1 <- lm(Sepal.Width ~ Sepal.Length + Petal.Length, data = iris1)
```

```{r fig.cap="2D-le kollapseeritud graafiline kujutus 3D andmete põhjal fititud mudelist. Muutuja Petal.Length on kollapseeritud konstandile.", message=FALSE, error=FALSE, fig.height=4, fig.width=6}
library(tidyverse)
...
Võrdluseks ühe prediktoriga mudel:
```{r echo=FALSE}
m <- lm(Sepal.Width ~ Sepal.Length, data = iris)
```

```{r fig.cap="2D visualiseering 2D andmete põhjal fititud mudelist.", echo=FALSE, fig.height=4, fig.width=6}
p + geom_abline(intercept = coef(m)[1], slope = coef(m)[2]) +
  labs(title = deparse(formula(m)))
```
...
Nagu näha, mudeli m b~1~ koefitsient erineb oluliselt mudeli m1 vastavast koefitsiendist. Ühel juhul on tõus nõrgalt negatiivne, teisel selgelt positiivne.

Kumb mudel on siis parem? AIC-i järgi on kahe prediktoriga m1 kõvasti eelistatum, kui ühe prediktoriga m.
```{r}
AIC(m, m1)
```
...
```{r fig.cap="Ennustus ainsa prediktoriga mudelist.", fig.height=3, fig.width=4}
...
```{r fig.cap="Ennustus 2 prediktoriga mudelist on sirge 3D ruumis.", echo=FALSE}
#library(rgl) #plot3d()
library(scatterplot3d)
Sepal_length <-  seq(min(iris$Sepal.Length), 
                     max(iris$Sepal.Length), 
...
```{r fig.cap="Ennustus interaktsioonimudelist, kus x1 on antud rida väärtusi ja x2 hoitakse konstantsena.", fig.height=3, fig.width=4}
Sepal_length <-  seq(min(iris$Sepal.Length), 
                     max(iris$Sepal.Length), 
                     length.out = 100) 
Petal_length <-  mean(iris$Petal.Length)
...
```{r fig.cap="Ennustused 3D interaktsioonimudelist üle paljude x1 ja x2 väärtuste.", echo=FALSE}
Sepal_length <-  seq(min(iris$Sepal.Length), 
                     max(iris$Sepal.Length), 
                     length.out = 100) 
Petal_length <-  seq(min(iris$Petal.Length), 
...
```{r fig.height=2, fig.width=4, message=FALSE, error=FALSE, fig.cap="Normaaljaotus tekib sõltumatutest efektidest."}
library(rethinking)
...
```{r fig.height=2, fig.width=4, fig.cap="Normaaljaotus tekib väikestest sõltuvatest efektidest."}
...
```{r fig.height=2, fig.width=4, fig.cap="Lognormaaljaotus tekib suurematest sõltuvatest efektidest."}
...
```{r fig.height=2, fig.width=4, fig.cap="Logaritmilises skaalas lognormaalsed efektid on normaaljaotusega."}
...
```{r message=FALSE, fig.height=3, fig.width=4, fig.cap="Normaaljaotus ja studenti t jaotus fitituna N = 3 andmetega. Juhuvalim normaaljaotusest, mille keskmine=0 ja sd=1 (andmepunktid on näidatud mustade munadena). Sinine joon - populatsioon, millest tõmmati valim; punane joon - normaaljaotuse mudel, mis on fititud valimi andmetel; must joon - Studenti t jaotuse mudel, mis on fititud samade andmetega.", echo=FALSE}
