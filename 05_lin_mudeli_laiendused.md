...
Kui x-l saavad olla ainult positiivsed väärtused, siis on logaritmimine sageli hea mõte. Andmetest peab kaotama ka nullid (või asendama mingi nullilähedase positiivse väärtusega). Näiteks seost sissetuleku ja oodatava eluea vahel on lihtsam ette kujutada kui eluiga ~ log(sissetulek). Log skaalas on beeta koefitsiendid peaaegu alati < 1. 

Erinevus naturaallogaritmi (log, põõrdlogaritm = exp(log(x))) ja log10 või log2 vahel on, et esimesel juhul on kergesti tõlgendatav beeta (aga mitte logaritmitud x-i väärtused) ja teisel juhul (log10 v log2) on kergesti tõlgendatavad logaritmitud x-i väärtused (aga mitte beeta). Seetõttu eelistavad loodusteadlased kasutada log2 ja log10 skaalasid, mida on mugavam otse log-skaalas tõlgendada. Kui te kasutate log2-e, siis üheühikuline muutus log skaalas on sama, mis kahekordne muutus meetrilises skaalas ja anti-logaritmi saab $2^{log2(x)}$. Log10 korral vastab ühe ühikuline muutus log skaalas 10-kordsele muutusele meetrilises skaalas ja anti-logaritmi saab $10^{log10(x)}$. NB! teie tulemuste (mudeli fiti) kvaliteedi koha pealt pole suurt vahet, millist logaritmi te kasutate -- olulised erinevused on "pelgalt" mudeli koefitsientide tõlgendustes.

Järgnevalt õpetame naturaallogaritmitud andmetega fititud mudelite β koefitsiendite tõlgendamist algeses, meetrilises skaalas.  

* y = a + bx lineaarne mudel

* y = a + b * log(x) linear-log mudel

* log(y) = a + bx log-linear mudel

* log(y) = a + b * log(x) log-log mudel

Kasutame naturaal-logaritmi, mille alus on e ≈ 2.71828.
...
**Lineaarses** mudelis Y = α + βX, annab β mitu ühikut muutub Y keskväärtus, kui X muutub ühe ühiku võrra. See tõlgendus jääb kehtima ka logaritmitud muutujate korral, ainult et log-ühikutes. Me võime siiski tahta tõlgendust mitte-logaritmitud ühikutes, vaid näiteks muutuse protsendina või lihtsalt algsetes meetrilistes ühikutes.
...
Selle muutuse saab avaldada protsendina: eX on sama, mis õelda, et X kasvab 172% võrra (100 × (2.72 − 1) = 172). 

* β on oodatud Y muutus kui X kasvab eX korda. 

* β on oodatud Y muutus kui X kasvab 172% võrra.

* Kui β on väike, siis saab seda otse tõlgendada kui suhtelist erinevust. Näiteks, kui beta = 0.06, siis võime seda tõlgendada nii: 1 ühikuline X-i muutus viib 6%-sele Y muutusele. Sedamõõda kui β kaugeneb nullist (näiteks β = 0.4), hakkab selline kiire hinnang tõsiselt alahindama tegelikku x-i mõju y väärtusele. 

* Oodatud Y muutus kui X kasvab p protsenti: β * log([100 + p]/100). Näit, kui X kasvab 10% võrra, siis log(110/100)= 0.095 ja 0.095 β on oodatud Y muutus kui x korrutada 1.1-ga (ehk kasvab 10% võrra). 

* Kui p on väike, siis log([100 + p]/100) ≈ p/100. Kui p = 1, siis β/100 on enam-vähem oodatud Y muutus, kui X kasvab 1 võrra. 
...
Kui mudel sisaldab binaarseid muutujaid, siis on kasulikum standardiseerida üle 2xSD, jättes binaarsed muutujad muutmata.
...
Ilma interaktsioonideta mudeli korral saab sama tulemuse, mis prediktoreid tsentreerides, kui me reskaleerime tavalises skaalas fititud mudeli koefitsiendid, korrutades iga β oma prediktori kahekordse sd-ga ($β.x = β * 2* sd(x)$). Nende β.x-de pealt näeb hästi iga muutuja suhtelist tähtsust mudelis. 
