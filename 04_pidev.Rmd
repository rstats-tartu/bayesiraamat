...
sim.length <- rethinking::sim( m8 , data=list(l_GDP=width) ) %>% as_tibble()
...
```{r}
m_ln1 <- map2stan(alist(
   gdpPercap  ~ dlnorm( mu , sigma ),
    mu <- a + b * lifeExp,
    a ~ dnorm(0, 10),
    b ~ dnorm(0, 10),
    sigma ~ dcauchy(0,2)
), data=g2007, start = list(a=3, b=0, sigma=0.5))
precis(m_ln1)
```
...
Ennustame score1 ja score2 summa sõltuvust sex-ist. Küsimus: kui palju poiste ja tüdrukute matemaatikaoskused erinevad? Fitime mudeli, mis laseb vabaks intercepti. **Selle mudeli eeldus on, et igal koolil on oma baas-tase (oma intercept), aga kõikide koolide efektid (mudeli tõusu-koefitsient) on identsed.**
