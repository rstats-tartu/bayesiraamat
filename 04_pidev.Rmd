...
Erineva SD-ga mudeli järgi on riigijuhtide pikkuste vahe `r round(median(samplesm2.1$ES), 1)` cm, ebakindlus väiksem -- 90% HDI on `r paste(round(HPDI(samplesm2.1$ES, prob = 0.9), 1), collapse = " kuni ")` ja tõenäosus et pikkuste erinevus on väiksem kui 0 on `r round(mean(samplesm2.1$ES < 0), 2)`.
...
Teiseks, koefitsient b = `r signif(coef(gapmod3)[2], 2)`, mis on üsna väike arv. See tähendab, et SKP tõus 1 USD võrra tõstab eluiga keskmiselt `r signif(coef(gapmod3)[2], 2)` aasta võrra (ja SKP tõus 1000 USD võrra tõstab eluiga `r signif(1000 * coef(gapmod3)[2], 2)` aasta võrra). 
Muidugi ainult siis, kui uskuda mudelit.

Kolmandaks, adjusted R squared on `r signif(summary(gapmod3)$r.squared, 2)`, mis tähendab et mudeli järgi seletab SKP varieerumine `r str_c(signif(100 * summary(gapmod3)$r.squared, 2), "%")` eluea varieeruvusest riikide vahel.

Hea küll, aga milline mudel on siis parim? 
```{r}
AIC(gapmod1, gapmod2, gapmod3)
```
...
Now, as it happens, in log scale the intercept-fixed-at-0 model is almost as good as the full model. 
This is, however, not a general feature of modelling.
...
```{r}
gcsemv <- read_delim("data/Gcsemv.txt", delim = " ", col_names = FALSE)
colnames(gcsemv) <- c("school", "student", "sex", "score1", "score2")
gcsemv <- gcsemv %>% mutate_at(vars(school, student), as.factor)
gcsemv$score1[gcsemv$score1 == -1] <- NA
gcsemv$score2[gcsemv$score2 == -1] <- NA
gcsemv <- drop_na(gcsemv)
gcsemv
```
...
```{r, include=FALSE}
# saveRDS(m2, file = "data/schoolm2.rds")
...
```{r, include=FALSE}
# saveRDS(m3, file = "data/schoolm3.rds")
m3 <- readRDS("data/schoolm3.rds")
```
...
```{r, include=FALSE}
# saveRDS(m4, file = "data/schoolm4.rds")
m4 <- readRDS("data/schoolm4.rds")
```
...
Siit nähtub, et m3 on parim mudel, aga ka m4 ja m2 omavad mingit kaalu.
...
schools <- schools %>%
  ...
  mutate(summa = score1 + score2,
         sex = as.factor(sex))
schools_f1 <- glimmer(summa ~ sex + (1 | school), data = schools)
```

```{r, eval=FALSE}
schools_m1 <- map2stan(flist = schools_f1$f, data = schools_f1$d) # siin tuleb kasutada glimmeri poolt loodud andmetabelit sest glimmer teeb factorid Stanile sõõdavaks
```
...
Ja selle soo tulemused on tõesti paremad kui 0-seksi omad. 
Intercept annab siin sex = 0 (poisid) keskmise  skoori kooli kaupa (kui liita üldisele interceptile kooli-spetsiifiline intercept). 
Kui tahame näiteks hinnangut 2. kooli tüdrukute skoorile (ehk tõelisele matemaatikavõimekusele) siis: 
...
schools_m1_intercept <- schools_m1_samples$Intercept + schools_m1_samples$b_sex1 + schools_m1_samples$`v_Intercept[2]`
## Plot density histogram of intercepts
dens(schools_m1_intercept)
```
...
```{r}
schools_f2 <- glimmer(summa ~ sex + (1 + sex | school), data = schools)
```
...
```{r, eval=FALSE}
schools_m2 <- map2stan(schools_f2$f, schools_f2$d)
```
...
dens(schools_m2_samples$Rho_school[, 1, 2])
```
...
```{r}
schools <- mutate_at(schools, "school", as.factor)
d.pred <- list(
  school = c(1, 2, 37),
  sex1 = 0
)

schools_sim <- rethinking::sim(m2, data = d.pred) 
...
Esimeses koolis jääb keskmine poiste skoor 89% tõenäosusega vahemikku `r str_c(round(pred.p.PI[,1]), collapse = " kuni ")` punkti.
...
schools_m2_samples_mean <- as.data.frame(schools_m2_samples_mean)
schools_m2_samples_mean$names <- colnames(schools_m2_samples)
schools_m2_samples_mean <- spread(schools_m2_samples_mean, 
                                  key = names, 
                                  value = schools_m2_samples_mean) #alternatiiv oleks regexpr kasutamine koos filter() verbiga.
schools_m2_samples_mean2 <- schools_m2_samples_mean %>% 
  select(starts_with("v_Int")) %>% 
  gather()
schools_m2_samples_mean3 <- schools_m2_samples_mean %>% 
  select(starts_with("v_sex")) %>% gather()

plot(schools_m2_samples_mean2$value, schools_m2_samples_mean3$value, 
     xlab = "Poiste skoor", 
     ylab = "Erinevus poiste ja tüdrukute vahel")
abline(lm(schools_m2_samples_mean3$value ~ schools_m2_samples_mean2$value))
