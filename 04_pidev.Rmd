...
# studenti t mudel, kus nu väärtus on fikseeritud (me ei arvuta seda üle nu priori)
m3 <- map2stan(
  alist(
    y ~ student_t(nu = 20, mu, sigma),
    mu ~ dnorm(0, 5), 
    sigma ~ dcauchy(0, 1)  
  ), 
  data = list(y = b),
...
Siin on m0 ilma outlierita mudel ja me tahame teada, milline mudel m1 kuni m3 sellele kõige lähedasemad tulemused annab (kuna m0 on fititud valimi andmete peal, ei ole ka mu täpselt 0 ja sigma täpselt 1). 
Nagu näha on m3 ehk fikseeritud nu-ga t-jaotus kõige lähemal m0-le ja seega parim mudel outlieriga andmetele. 
```{r}
...
Ilusamad parameetriplotid saate *bayesplot* raamatukogu kasutades.
```{r}
library(bayesplot)
...
Pane tähele, et kuna me fikseerisime nu väärtuse tõepäramudelis, pole sellele enam priorit vaja sets meie Bayesi arvutusmasin ei määra enam ise nu väärtust. 
Nu otsene hindamine töötab alati üsna ebakindalt: 
