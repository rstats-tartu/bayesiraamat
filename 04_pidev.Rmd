...
```{r}
set.seed(1)
a <- rnorm(20) # expected mean = 0, sd = 1
b <- c(a, 5, 9) # plus 2 outliers
mean(a); sd(a)
mean(b); sd(b)
```

Siin kasutame andmeid, mille keskväärtus on 0.19 ja sd = 0.91 ja millele on lisatud kaks outlierit (3 ja 9). Proovime neid andmeid mudeldada normaaljaotusega tõepäramudeliga ja seejärel üle studenti t jaotuse. Me fitime 4 mudelit, neist 3 koos outlieritega. Mudeli fittimine käib nii, et mcmc ahelad sammuvad parameetriruumis ja iga samm annab meile ühe väärtuse posteeriorist. Defaultina on meil üks ahel, mis teeb 1000 sammu (seda saab muidugi muuta. vt ?map2stan). Kuna ahelad veedavad rohkem aega seal, kus posterioorne tõenäosuspilv on tihedam, siis saab salvestatud numbrite histogrammist posterioorse jaotuse kuju. Veelgi enam, selle asemel, et tegeleda posterioorsete jaotuste matemaatilise analüüsiga (integreerimisega) võime analüüsida oma mcmc sämpleid otse, mis tähendab, et kõrgema matemaatika asemel vajame 2. klassi aritmeetikat.
...
m3 <- map2stan(
  alist(
    y ~ student_t( nu = 4, mu, sigma ),
    mu ~ dnorm( 0, 5 ), 
    sigma ~ dcauchy( 0, 1 )  
  ), 
  data = list( y = b ), start = list ( mu=mean( b ), sigma=sd( b ) ) )
```
...
Kui Rhat on 1, siis see tähendab, et MCMC ahelad on ilusti jooksnud ja posteeriori sämplinud. Kui Rhat > 1.1, siis on kuri karjas. Suur Rhat viitab, et ahel(ad) pole jõudnud konvergeeruda. Aitab see, kui pikendada warm-up perioodi (näiteks: map2stan(..., iter= 3000, warmup=2000) pikendab warm-upi 2 korda). 
...
Funktsiooniga `extract.samples()` saame koos sämplitud parameetrite numbrid kõrvuti tabeli rea kaupa. 
...
```{r}
filter(s1, between(CV, 2, 5)) %>% nrow/nrow(s1)
```

Vastus on, et me arvame 39% kindlusega, et tõde jääb kuskile sellesse vahemikku.
...
```{r}
plot(coeftab(m0, m1, m2, m3), pars = c("mu", "sigma"), prob=0.5)
```
...
df1$indeks <- rep("USA", times=nrow(df1)) 
#selle abil jagame hiljem koondtabeli kahte ossa
world_presidents <- read.csv2("data/world_presidents.csv")
world_presidents$indeks <- rep("eur", times=nrow(world_presidents)) 
#selle abil jagame hiljem koondtabeli kahte ossa
...
m2 <- map2stan(
    alist(
        ...
        mu <- mu1[indeks], #mu is redifined as mu1, which takes values at each indeks level. Now its mu1[indeks], not mu, that needs a prior.
...
m2.1 <- map2stan(
    alist(
        ...
        sigma <- sigma1[indeks],
        mu1[indeks] ~ dnorm(0,200), # normal prior for mean
        sigma1[indeks] ~ dcauchy(0,10) #half-cauchy prior from sd 
), data=df2, start = list( mu1 = c( mean( df2$heights[df2$indeks == "USA"] ), 
                                    mean( df2$heights[df2$indeks == "eur"] ) ), 
                           sigma1= c( sd( df2$heights[df2$indeks == "USA"] ), 
                                      sd(df2$heights[df2$indeks=="eur"] ) ) ) 
)
```
