...
1. Kui hüpotees A sisaldab endas hüpoteesi B, siis $P(A) <= P(B)$. 

2. kui A ja B on üksteist välistavad, siis tõenäosus, et A on tõene või B on tõene on nende kahe tõenäosuse summa --- $P(A \lor B) = P(A) + P(B)$. 

3. kui A ja B on üksteisest sõltumatud siis $P(A & B) = P(A)P(B)$, kui A ja B ei ole üksteist välistavad, siis $P(A v B) = P(A) + P(B) – P(A & B)$. 
...
Bayesi statistika opereerib episteemilise tõenäosusega. See tähendab, et tõenäosus annab numbrilise mõõdu meie ebakindluse määrale mõne hüpoteesi ehk parameetriväärtuse kehtimise kohta. Seega mõõdab tõenäosus meie teadmiste kindlust (või ebakindlust, kui soovite). Näiteks, kui Bayesi arvutus väidab, et vihma tõenäosus homme on 60%, siis me oleme 60% kindlad, et homme tuleb vihma. Aga hoolimata sellest, mida me vihma kohta usume, homme kas sajab vihma või mitte, ja seega on objektiivne vihma tõenäosus meie akna taga 0% või 100% -- mitte kunagi 60%. 
...
myfun <- function(x) summarise(x, Mean = mean(value))
boot_mean <- heights %>% 
  bootstrap(1000) %>% 
  do(M = myfun(.)) %>% 
  unnest
...
# a1 <- a %>% 
#   group_by(key) %>% 
#   summarise(Value = mean(value)) # Calculate the mean for each slice of n values
# dens(a1$Value)
```
...
a1 <- a %>% 
  group_by(key) %>% 
  summarise(Value = mean(value) - mean(heights$value)) # calculate the difference between the mean for each slice of n values and the emprirical sample mean
dens(a1$Value)
```
...
# Plot posterior
plot(p_grid , posterior , type = "b", xlab = "Mortality" , ylab = "Posterior probability", main = "Posterior distribution" )
par(def.par)
```
...
plot(p_grid , posterior, type = "b" , xlab = "Mortality", ylab = "Posterior probability" )
```
...
plot(p_grid , posterior1, type = "b", xlab = "Mortality" , ylab = "Posterior probability" )
```
...
plot(p_grid, posterior2, type = "b", xlab = "Mortality", ylab = "Posterior probability")
```
...
def.par <- par(no.readonly = TRUE)
par(mfrow=c(1,2))
plot(1:20, prior, type = "b", main = "Prior")
plot(p_grid, posterior, type = "b", xlab = "Mortality", ylab = "Posterior probability", main = "Posterior")
par(def.par)
```
1. patsient suri
...
par(mfrow=c(1,2))
plot(1:20, prior, type = "b", main = "Prior")
plot(p_grid, posterior1, type = "b", xlab = "Mortality", ylab = "Posterior probability", main = "Posterior")
par(def.par)
```
