...
```{r vrdlssd, fig.cap="Paariviisiline katse - kontroll disain. Katset on korratud 30 korda. X-teljel on efektisuurused (ES). 30 üksikut efektisuurust on näidatud punktidena. Must joon näitab keskmist efektisuurust. Andmed on mudeldatud normaaljaotusena.", echo=FALSE, fig.height=3, fig.width=4}
set.seed(19)
...
```{r vrdlsse, fig.cap="See jaotus iseloomustab keskmise ES paiknemist puhtalt meie andmete põhjal.", echo=FALSE, fig.height=3, fig.width=4}
p + geom_dotplot(binwidth = 1, dotsize = 1/3) + 
  stat_function(fun = densfun,
...
```{r vrdlssdse, fig.cap="Taustateadmiste mudel ehk prior on normaaljaotus (must joon), mille ülesanne on veidi vähendada ekstreemsete valimite kahjulikku mõju.", echo=FALSE, fig.height=3, fig.width=4}
...
```{r vrdlsstan, cache=TRUE, eval=FALSE}
dfa <- data.frame(a)
m99 <- map2stan(
  alist(
...
```{r vrdlsnull, fig.cap="Nullhüpotees (must kõver) ja tõepärafunktsioon (punane kõver).", echo=FALSE, fig.height=3, fig.width=4}
p + stat_function(fun = densfun,
                args = list(data = a, se = TRUE),
...
NB! Valimiviga ja andmete esinduslikus on erinevad ja üksteisest sõltumatud asjad, ehkki mõlemad on juhuslikud protsessid, mille tõenäosus kahaneb andmete hulga kasvuga. Kui valim on piisavalt suur, siis võime olla piisavalt kindlad, et andmed on esinduslikud ning klassikalise statistika hinnangud ebakindlusele punktväärtuse ümber muutuvad selle võrra usutavamaks.  
