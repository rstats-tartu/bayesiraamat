...
regular expression võimaldab näiteks plottida mudeli madalama taseme posteerioreid, mida plot tavaliselt ei näita.
```{r}
#regex works!
...
```{r}
stanplot(m1, pars = "r_", type = "intervals")
```

type= argument sisestamine kujul type = "intervals" võimaldab plottida erinevaid diagnostilisi näitajaid type = "intervals" type argumendi lubatud sisendid on hist, dens, hist_by_chain, dens_overlay, violin, intervals, areas, acf, acf_bar,trace, trace_highlight, scatter, rhat, rhat_hist, neff, neff_hist nuts_acceptance, nuts_divergence, nuts_stepsize, nuts_treedepth ja nuts_energy.

posteeriorid ahela kaupa:
```{r}
stanplot(m1, type = "dens_overlay")
```
...
```{r}
stanplot(m1, pars = "r_|b_|sd|sigma")
```

Aga bayesplot::mcmc_intervals() näitab ka neid. Selleks, et oma mudel sellesse funktsiooni suunata, peame kõigepealt oma mudeli fiti (koefitsientide posterioorsed valimid) andmeraamiks konverteerima.
...
```{r eval=FALSE}
m4 <- brm(Sepal.Length~Petal.Length + Sepal.Width + Petal.Length*Sepal.Width, 
          data= iris, 
          prior = prior, 
          family = gaussian,
          warmup = 1000, 
...
plot(marginal_effects(m4, effects = "Petal.Length", conditions = conditions, re_formula = NULL), points= TRUE)
```
