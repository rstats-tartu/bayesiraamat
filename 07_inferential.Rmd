...
```{r fig.cap="Keskmiste jaotus 10 000 valimist."}
N <- 3
N_simulations <- 10000
...
```{r fig.cap="SD-de jaotus 10 000 valimist."}
Summary %>% 
  ggplot(aes(SD)) + 
  geom_histogram(bins = 40)
```
...
```{r, out.width='.49\\linewidth', fig.cap="Bootstrapitud posteerior USA presidentide keskmisele pikkusele."}
library(rethinking)
...
```{r fig.cap="Empiirilise bootstrapi posteerior USA presidentide keskmisele pikkusele.", fig.height=3, fig.width=3}
boot_mean <- boot_mean %>% 
...
```{r fig.cap="Bayesi bootstrapi posteerior USA presidentide keskmisele pikkusele."}
library(bayesboot)
...
```{r fig.cap="Bayesi bootstrap ES-le."}
set.seed(1)
...
```{r fig.cap="Parameetrilise bootstrapi posteerior USA presidentide keskmisele pikkusele."}
...
```{r, fig.align='center', echo=FALSE, fig.cap="x"}
...
```{r fig.cap="Tõepärafunktsioon binoomjaotuse mudelist."}
plot(x, y, 
     type = "b",
     xlab = "Number of deaths",
...
```{r fig.cap="Veel üks tõepärafunktsioon."}
plot(x, y, 
     type = "b", 
     xlab = "Number of deaths", 
     ylab = "Probability of x deaths", 
...
```{r fig.cap="N=1, esimene patsient suri."}
# Define grid
x <- seq(from = 0, to = 1, length.out = 20)
...
```{r fig.cap="N=2, teine patsient suri."}
# Define prior
prior <- posterior
...
```{r fig.cap="N=3, kolmas patsient jäi ellu."}
# Define prior
prior <- posterior1
...
```{r fig.cap="tõmbame valimi posteeriorist."}
...
```{r fig.cap="Veel üks valim posteeriorist (60 surma 90st)."}
...
```{r, out.width='0.49\\linewidth', fig.cap="Posteerior, mis on arvutatud beta prioriga binoomsest tõepäramudelist."}
dens(samples$p)
HPDI(samples$p, prob = 0.95) # Highest density 95% at the center
```
