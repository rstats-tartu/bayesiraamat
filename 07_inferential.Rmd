...
6. Tingliku tõenäosause definitsioon: $P(A~\vert~B) = P(A \land B)/P(B)$ kus $P(A~\vert~B)$ on tinglik tõenäosus: A tõenäosus tingimusel, et B kehtib. $P(vihm~\vert~pilves~ilm)$ ei ole sama, mis $P(pilves~ilm~\vert~vihm)$. 

7. Punktist 6 tuleneb: $P(A \land B) = P(A~\vert~B)P(B)$.

8. Punktist 6 tuleneb totaalne tõenäosus: $P(A) = P(A~\vert~B)P(B) + P(A~\vert~notB)P(notB)$ 

9. Punktist 6 tuleneb Bayesi teoreem: $P(A~\vert~B) = P(A)P(B~\vert~A)/P(B)$, kus $P(B) = P(A)P(B~\vert~A) + P(not~A)P(B~\vert~not~A)$  
...
$P(H1~\vert~data) =  P(H1)P(data~\vert~H1) /( P(H1)P(data~\vert~H1) + P(H2)P(data \vert H2) )$
...
1. H1 tõepära, ehk tõenäosus saada positiivne mammogramm juhul, kui patsiendil on rinnavähk $P( +~\vert~H_1) = 0.9$

2. H2 tõepära, ehk tõenäosus saada positiivne mammogramm juhul, kui patsiendil ei ole rinnavähki $P( +~\vert~H_2) = 0.08$ (pane tähele, et 0.9 + 0.08 ei võrdu ühega, mis tähendab, et tõepära pole tõenäosusteooria mõttes päris tõenäosus).

3. Eelnev tõenäosus, et patsiendil on rinnavähk $P(H_1) = 0.01$ (see on rinnavähi sagedus 45 a naiste populatsioonis; kui me teame patsiendi genoomi järjestust või rinnavähijuhte tema lähisugulastel, võib P(H1) tulla väga erinev).
...
```{r, out.width='.49\\linewidth', fig.cap="Bootstrapitud posteerior USA presidentide keskmisele pikkusele.",  fig.height=2, fig.width=3}
library(rethinking)
...
```{r fig.cap="Empiirilise bootstrapi posteerior USA presidentide keskmisele pikkusele.", fig.height=3, fig.width=4}
boot_mean <- boot_mean %>% 
...
```{r fig.cap="Bayesi bootstrapi posteerior USA presidentide keskmisele pikkusele.", fig.height=4, fig.width=5}
library(bayesboot)
...
```{r fig.cap="Bayesi bootstrap ES-le.", fig.height=4, fig.width=5}
set.seed(1)
...
```{r fig.height=4, fig.width=5, fig.cap="Parameetrilise bootstrapi posteerior USA presidentide keskmisele pikkusele."}
...
```{r, fig.align='center', echo=FALSE, fig.cap="Bootstrappimise meetodid.", echo=FALSE}
...
```{r fig.cap="Tõepärafunktsioon.", fig.height=4, fig.width=5}
plot(x, y, 
...
```{r fig.cap="Tõepärafunktsioon binoomjaotuse mudelist.", fig.height=4, fig.width=5}
plot(x, y, 
     type = "b",
     xlab = "Number of deaths",
...
```{r fig.cap="Veel üks tõepärafunktsioon.", fig.height=4, fig.width=5}
plot(x, y, 
     type = "b", 
     xlab = "Number of deaths", 
     ylab = "Probability of x deaths", 
...
```{r fig.cap="Posteerior.", fig.height=4, fig.width=5}
# Define grid
x <- seq(from = 0 , to = 9)
# Define flat prior
prior <- rep(1 , 10)
...
```{r fig.cap="Prior, tõepära ja posteerior."}
...
```{r fig.cap="N=1, esimene patsient suri.", fig.height=4, fig.width=5}
# Define grid
x <- seq(from = 0, to = 1, length.out = 20)
...
```{r fig.cap="N=2, teine patsient suri.", fig.height=4, fig.width=5}
# Define prior
prior <- posterior
...
```{r fig.cap="N=3, kolmas patsient jäi ellu.", fig.height=4, fig.width=5}
# Define prior
prior <- posterior1
...
```{r fig.cap="N=1 informatiivse prioriga."}
# Define prior
...
```{r fig.cap="N=2 informatiivse prioriga."}
# Define prior
prior <- posterior
...
```{r fig.cap="N=3 informatiivse prioriga."}
# Define prior
prior2 <- posterior1
