...
Me anname siin 9 tuletust ilma tõestuskäikudeta, mis on aga lihtsad. Siin võib A ja B vaadelda erinevate sündmustena või hüpoteesidena. $P(A \vert B)$ on tinglik tõenäosus, mida tuleks lugeda nii: "A tõenäosus tingimusel, et kehtib B". $P(vihm~\vert~pilves~ilm)$ ei ole sama, mis $P(pilves~ilm~\vert~vihm)$. $P(A \land B)$ tähendab A ja B, $P(A \lor B)$ tähendab A ja/või B (loogiline "või" tähendab tavakeeles ja/või).
...
5. $P(A) = P(A \land B) + P(A \land notB)$ -- see on "totaalse tõenäosuse seadus" kitsendatuna kahele ammendavale teineteist välistavale hüpoteesile, mille kohaselt me peame hüpotees A tõenäosuse leidmiseks arvestama ka B tõenäosusega.

6. Tingliku tõenäosause definitsioon: $P(A~\vert~B) = P(A \land B)/P(B)$  
