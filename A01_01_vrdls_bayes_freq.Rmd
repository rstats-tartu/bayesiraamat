...
```{r fig.cap="Paariviisiline katse - kontroll disain. Katset on korratud 30 korda. X-teljel on efektisuurused (ES). 30 üksikut efektisuurust on näidatud punktidena. Must joon näitab keskmist efektisuurust. Andmed on mudeldatud normaaljaotusena."}
set.seed(19)
...
```{r, fig.cap="See jaotus iseloomustab keskmise ES paiknemist puhtalt meie andmete põhjal."}
p + geom_dotplot(binwidth = 1, dotsize = 1/3) + 
  stat_function(fun = densfun,
...
```{r, fig.cap="Taustateadmiste mudel ehk prior on normaaljaotus (must joon), mille ülesanne on veidi vähendada ekstreemsete valimite kahjulikku mõju."}
...
ggplot(mm, aes(mu))+ 
  ...
  stat_function( fun=dnorm, args=list(mean=mean(tib$a),sd=sd(tib$a)/sqrt(30)), colour="red") +
...
```{r echo=FALSE}
tib %>% ggplot(aes(x=a)) +
  stat_function(fun=dnorm, args=list(mean=mean(tib$a),sd=sd(tib$a)/sqrt(30)), colour="red", alpha=0.2) +
  stat_function(fun=dnorm, args=list(mean=0,sd=sd(tib$a)/sqrt(30))) +
  geom_vline(aes(xintercept=0), linetype=2)+
  geom_vline(aes(xintercept= mean(tib$a)))+
